...
def update_experiment(experiment, step, loss, top1, lr):
    experiment.log_metric('top_1', top1.val, step=step)
    experiment.log_metric('loss', loss.val, step=step)
    experiment.log_metric('lr', lr, step=step)

def kd_vanilla(epoch, model, teacher_model, optimizer, dataloader, experiment, params):
    model.train()
    teacher_model.eval()
...
def kd_mixup(epoch, model, teacher_model, optimizer, dataloader, experiment, params):
    ...
    for i, (train_batch, labels_batch) in enumerate(pbar):
        ...
        if params.loss_func in ['loss_fn_kd_mixup_origin', 'loss_fn_kd_mixup_correction', 'loss_fn_kd_mixup_correction_plus']:
